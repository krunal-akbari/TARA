...
import { getResumeContent, getResumePreviewText, listResumes, uploadResume } from "@/lib/services/resumes";
...
const ALL_HR_INLINE_FIELDS: Array<keyof HrInlineState> = [
  ...HR_INLINE_GENERAL_FIELDS,
  ...HR_INLINE_STATUS_FIELDS,
  ...HR_INLINE_PAY_FIELDS,
];
...
export default function CandidateDetailPage() {
  ...
  const [hrNotes, setHrNotes] = useState<HrNotesState>({
    general: "",
    status: "",
    pay: "",
    notes: "",
  ...
  const [hrInline, setHrInline] = useState<HrInlineState>({
    ...
    specialDisabledVeteran: false,
    vietnamEraVeteran: false,
    otherProtectedVeteran: false,
    nationalGuard: false,
    milSepDate: "",
    i9Verified: "",
    renewDate: "",
  ...
  const { data: jobsData, isLoading: jobsLoading } = useQuery({
    ...
    queryFn: () =>
      listJobs({
        page: 1,
        pageSize: 50,
        search: normalizedJobSearch || undefined,
      }),
  ...
  const applyMutation = useMutation({
    ...
    onError: (err) => {
      setActionsFeedback(getApiErrorMessage(err, "Failed to apply candidate to job"));
    },
  });
  ...
  useEffect(() => {
    if (!canViewHrNotes && activeTab === "hr_notes") {
  ...
  const tabs = useMemo(
    () => {
      const tabItems: Array<{ id: CandidateTabId; label: string }> = [
      ...
      if (canViewHrNotes) {
        tabItems.push({ id: "hr_notes", label: "HR" });
      }
      return tabItems;
    },
    [canViewHrNotes],
  ...
  const clientIds = useMemo(
    () => Array.from(new Set(filteredJobs.map((job) => job.origin_client_id).filter((value): value is number => value !== null))),
    [filteredJobs],
  );
  const vendorIds = useMemo(
    () => Array.from(new Set(filteredJobs.map((job) => job.origin_vendor_id).filter((value): value is number => value !== null))),
    [filteredJobs],
  );
  const clientNameQueries = useQueries({
    queries: clientIds.map((clientId) => ({
      queryKey: queryKeys.clients.detail(clientId),
      queryFn: () => getClient(clientId, true),
      enabled: actionsOpen,
      staleTime: 5 * 60 * 1000,
    })),
  });
  ...
  const clientNameMap = useMemo(() => {
    ...
    for (let index = 0; index < clientIds.length; index += 1) {
      const client = clientNameQueries[index]?.data;
      if (client) entries.push([clientIds[index], client.name]);
    }
  ...
  const vendorNameMap = useMemo(() => {
    ...
    for (let index = 0; index < vendorIds.length; index += 1) {
      const vendor = vendorNameQueries[index]?.data;
      if (vendor) entries.push([vendorIds[index], vendor.name]);
    }
    return new Map<number, string>(entries);
  ...
  const applicationStatusCounts = useMemo(() => {
    return (candidateApplicationsData?.items ?? []).reduce<Record<string, number>>((counts, application) => {
  ...
  useEffect(() => {
    ...
    const onPointerDown = (event: MouseEvent | TouchEvent) => {
      ...
      if (target && !actionsPanelRef.current.contains(target)) {
  ...
  const onApplyToJob = async () => {
    if (!selectedJob) {
      setActionsFeedback("Select one job first.");
      return;
    }
    ...
    <div className="space-y-4">
      ...
      {data ? (
        <>
            ...
            <div className="border-b border-slate-200 px-4 py-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div ref={actionsPanelRef} className="relative flex items-center gap-2">
                  ...
                  {actionsOpen ? (
                    <div className="absolute right-0 top-full z-20 mt-2 w-[26rem] rounded border border-slate-300 bg-white p-3 shadow-xl">
                      ...
                      <div className="mt-2 max-h-56 overflow-auto rounded border border-slate-200">
                        {jobsLoading ? (
                          <p className="px-3 py-2 text-sm text-slate-600">Loading jobs...</p>
                        ) : null}

                        {!jobsLoading && filteredJobs.length === 0 ? (
                          <p className="px-3 py-2 text-sm text-slate-600">No jobs match your search.</p>
                        ) : null}
                        ...
                        {!jobsLoading
                          ? filteredJobs.map((job) => (
                            <button
                              ...
                              className={cn(
                                "flex w-full items-center justify-between border-b border-slate-200 px-3 py-2 text-left text-sm last:border-b-0",
                              ...
                              <span className="min-w-0 pr-3">
                                ...
                                <span className="block truncate text-xs text-slate-500">
                                  {(() => {
                                    ...
                                    if (job.origin_client_id) {
                                      const clientName = clientNameMap.get(job.origin_client_id) ?? `Client #${job.origin_client_id}`;
                                      hostParts.push(`Direct Client: ${clientName}`);
                                    }
                                    if (job.origin_vendor_id) {
                                      const vendorName = vendorNameMap.get(job.origin_vendor_id) ?? `Vendor #${job.origin_vendor_id}`;
                                      hostParts.push(`Vendor: ${vendorName}`);
                                    }
                                    return hostParts.length > 0 ? hostParts.join(" | ") : "No host assigned";
                                  })()}
                                </span>
                              </span>
                            </button>
                      ...
                      {actionsFeedback ? (
                        <p className="mt-2 text-xs text-slate-600">{actionsFeedback}</p>
                      ) : null}
              ...
              <div className="border-b border-slate-200 bg-white px-4 py-3">
                <div className="grid grid-cols-5 gap-2">
                  {headerStats.map((stat) => (
                    <div key={stat.label} className="rounded border border-slate-200 bg-white px-2 py-2 text-center">
                  ...
                  {!candidateApplicationsLoading && (candidateApplicationsData?.items?.length ?? 0) > 0 ? (
                    <div className="space-y-2">
                      {(candidateApplicationsData?.items ?? []).map((application) => {
                        ...
                        const statusOptions = APPLICATION_STATUSES.includes(
                          normalizedCurrentStatus as (typeof APPLICATION_STATUSES)[number],
                        )
                        ...
                        return (
                          <div key={application.id} className="rounded border border-slate-200 bg-slate-50 px-3 py-2 text-sm">
                            ...
                            <p className="mt-1 text-xs text-slate-600">Job ID #{application.job_id}</p>
                            <div className="mt-2">
                            ...
                            <Link href={`/jobs/${application.job_id}`} className="mt-2 inline-flex text-xs font-medium text-blue-700 hover:underline">
                              Open Job
                            </Link>
                          </div>
                        );
          ...
          {activeTab === "notes" ? (
            <Card className="space-y-3">
              <p className="text-base font-semibold text-slate-900">Activity</p>
              {candidateApplicationsLoading ? (
              ...
              {!candidateApplicationsLoading && (candidateApplicationsData?.items?.length ?? 0) > 0 ? (
                <div className="overflow-x-auto">
                  <table className="min-w-full text-sm">
                    ...
                    <tbody>
                      {(candidateApplicationsData?.items ?? []).map((item) => (
                        <tr key={item.id} className="border-b border-slate-100">
                          <td className="px-2 py-2 text-slate-700">{item.created_at}</td>
                          <td className="px-2 py-2 text-slate-900">{item.job_title}</td>
                          <td className="px-2 py-2"><StatusChip value={item.status} /></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {activeTab === "files" ? (
            <Card className="space-y-4">
              <div>
                ...
                <p className="text-sm text-slate-600">Add files for this candidate.</p>
              </div>
              ...
              <div className="rounded border border-slate-200 bg-slate-50 p-3">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  ...
                  <Button
                    type="button"
                    onClick={() => selectedUploadFile && uploadResumeMutation.mutate(selectedUploadFile)}
                    disabled={!selectedUploadFile || uploadResumeMutation.isPending}
                  >
              ...
              <div className="rounded border border-slate-200 bg-white p-3">
                ...
                <div className="mt-2 overflow-x-auto">
                  <table className="min-w-full text-sm">
                    ...
                    <tbody>
                      {resumesLoading ? (
                        <tr><td className="px-2 py-3 text-slate-600" colSpan={4}>Loading files...</td></tr>
                      ) : null}
                      {!resumesLoading && (resumesData?.items?.length ?? 0) === 0 ? (
                        <tr><td className="px-2 py-3 text-slate-600" colSpan={4}>No files uploaded yet.</td></tr>
                      ) : null}
                      {(resumesData?.items ?? []).map((resume) => (
                        <tr key={resume.id} className="border-b border-slate-100">
                          <td className="px-2 py-2 text-slate-900">{resume.file_name}</td>
                ...
                <div className="mt-3">
                  <Link href={`/candidates/${id}/resumes`} className="text-sm font-medium text-blue-700 hover:underline">
                    Open Resume Manager
                  </Link>
                </div>
              </div>
            ...
            <Card className="overflow-hidden p-0">
              <div className="space-y-4 px-4 py-4">
                <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                  ...
                  <Button
                    type="button"
                    onClick={() => saveHrNotesMutation.mutate()}
                    disabled={!isHrNotesDirty || saveHrNotesMutation.isPending}
                  >
                ...
                {hrNotesTab === "general" ? (
                  <section className="space-y-3">
                    ...
                    <div className="grid gap-4 lg:grid-cols-[1.05fr_1fr]">
                        <div className="space-y-4">
                          <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                            ...
                            <div className="mt-2 grid grid-cols-4 gap-3 text-sm">
                              <div>
                                ...
                                <p className="tabular-nums font-medium text-slate-900">{data.id}</p>
                              </div>
                              <div>
                                <p className="text-xs font-semibold text-slate-600">First</p>
                                <p className="font-medium text-slate-900">{firstName || "-"}</p>
                              </div>
                              <div>
                                <p className="text-xs font-semibold text-slate-600">Middle</p>
                                <p className="font-medium text-slate-900">{editMeta.middleName || "-"}</p>
                              </div>
                              <div>
                                <p className="text-xs font-semibold text-slate-600">Last</p>
                          ...
                          <div className="grid gap-3 sm:grid-cols-2">
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Alias</Label>
                              <Input
                                className="mt-1 h-9"
                                value={hrInline.alias}
                                onChange={(event) => setHrInline((prev) => ({ ...prev, alias: event.target.value }))}
                              />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Salutation</Label>
                              <Input
                                className="mt-1 h-9"
                                value={hrInline.salutation}
                                onChange={(event) => setHrInline((prev) => ({ ...prev, salutation: event.target.value }))}
                              />
                            </div>
                          </div>
                          ...
                          <div className="grid gap-3 sm:grid-cols-2">
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Home</Label>
                              <Input
                                className="mt-1 h-9"
                                value={hrInline.homePhone}
                                onChange={(event) => setHrInline((prev) => ({ ...prev, homePhone: event.target.value }))}
                              />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Work</Label>
                              <Input className="mt-1 h-9" value={editMeta.workPhone || ""} readOnly />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Cell/Pager</Label>
                              <Input className="mt-1 h-9" value={phone || ""} readOnly />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Fax</Label>
                              <Input
                                className="mt-1 h-9"
                                value={hrInline.fax}
                                onChange={(event) => setHrInline((prev) => ({ ...prev, fax: event.target.value }))}
                              />
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                            ...
                            <div className="mt-2 grid gap-3 sm:grid-cols-2">
                              <div className="sm:col-span-2">
                                <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">SSN</Label>
                                <Input className="mt-1 h-9" value={hrInline.ssn} onChange={(event) => setHrInline((prev) => ({ ...prev, ssn: event.target.value }))} />
                              </div>
                              <div className="sm:col-span-2">
                                <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Driver License #</Label>
                                <Input className="mt-1 h-9" value={hrInline.driverLicense} onChange={(event) => setHrInline((prev) => ({ ...prev, driverLicense: event.target.value }))} />
                              </div>
                              <div>
                                <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Birthdate</Label>
                                <Input type="date" className="mt-1 h-9" value={hrInline.birthdate} onChange={(event) => setHrInline((prev) => ({ ...prev, birthdate: event.target.value }))} />
                              </div>
                              <div>
                                <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Age</Label>
                                <Input className="mt-1 h-9" value={hrInline.age} onChange={(event) => setHrInline((prev) => ({ ...prev, age: event.target.value }))} />
                              </div>
                              <div>
                                <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Race</Label>
                                <Input className="mt-1 h-9" value={hrInline.race} onChange={(event) => setHrInline((prev) => ({ ...prev, race: event.target.value }))} />
                              </div>
                              <div>
                                <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Gender</Label>
                                <Input className="mt-1 h-9" value={hrInline.gender} onChange={(event) => setHrInline((prev) => ({ ...prev, gender: event.target.value }))} />
                              </div>
                              <div className="sm:col-span-2">
                                <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Marital</Label>
                                <Input className="mt-1 h-9" value={hrInline.marital} onChange={(event) => setHrInline((prev) => ({ ...prev, marital: event.target.value }))} />
                              </div>
                              <div className="sm:col-span-2 grid grid-cols-2 gap-3 pt-1">
                                <label className="flex items-center gap-2 rounded border border-slate-200 bg-white px-2 py-2 text-sm text-slate-800">
                                  <input type="checkbox" checked={hrInline.disabled} onChange={(event) => setHrInline((prev) => ({ ...prev, disabled: event.target.checked }))} />
                                  Disabled
                                </label>
                                <label className="flex items-center gap-2 rounded border border-slate-200 bg-white px-2 py-2 text-sm text-slate-800">
                                  <input type="checkbox" checked={hrInline.smoker} onChange={(event) => setHrInline((prev) => ({ ...prev, smoker: event.target.checked }))} />
                                  Smoker
                                </label>
                              </div>
                            </div>
                    ...
                    <Textarea
                      ...
                      value={hrNotes.general}
                      onChange={(event) => setHrNotes((prev) => ({ ...prev, general: event.target.value }))}
                      placeholder="General HR notes..."
                    />
                  </section>
                ) : null}
                  ...
                  <section className="space-y-3">
                    ...
                    <div className="grid gap-4 lg:grid-cols-2">
                        <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                          ...
                          <div className="mt-2 grid gap-3 sm:grid-cols-2">
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Original Hire</Label>
                              <Input className="mt-1 h-9" value={editMeta.employmentStartDate || ""} readOnly />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Last Hired</Label>
                              <Input className="mt-1 h-9" value={editMeta.projectStartDate || editMeta.employmentStartDate || ""} readOnly />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Adjusted Seniority</Label>
                              <Input className="mt-1 h-9" value={editMeta.employmentStartDate || ""} readOnly />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Tenure</Label>
                              <Input className="mt-1 h-9" value={seniorityYears} readOnly />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Recruiter</Label>
                              <Input className="mt-1 h-9" value={editMeta.bdm || ""} readOnly />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Source</Label>
                              <Input className="mt-1 h-9" value={editMeta.source || ""} readOnly />
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                          ...
                          <div className="mt-2 grid gap-2">
                            <label className="flex items-center gap-2 rounded border border-slate-200 bg-white px-2 py-2 text-sm"><input type="checkbox" checked={hrInline.militaryVeteran} onChange={(event) => setHrInline((prev) => ({ ...prev, militaryVeteran: event.target.checked }))} />Military Veteran</label>
                            <label className="flex items-center gap-2 rounded border border-slate-200 bg-white px-2 py-2 text-sm"><input type="checkbox" checked={hrInline.specialDisabledVeteran} onChange={(event) => setHrInline((prev) => ({ ...prev, specialDisabledVeteran: event.target.checked }))} />Special Disabled Veteran</label>
                            <label className="flex items-center gap-2 rounded border border-slate-200 bg-white px-2 py-2 text-sm"><input type="checkbox" checked={hrInline.vietnamEraVeteran} onChange={(event) => setHrInline((prev) => ({ ...prev, vietnamEraVeteran: event.target.checked }))} />Vietnam Era Veteran</label>
                            <label className="flex items-center gap-2 rounded border border-slate-200 bg-white px-2 py-2 text-sm"><input type="checkbox" checked={hrInline.otherProtectedVeteran} onChange={(event) => setHrInline((prev) => ({ ...prev, otherProtectedVeteran: event.target.checked }))} />Other Protected Veteran</label>
                            <label className="flex items-center gap-2 rounded border border-slate-200 bg-white px-2 py-2 text-sm"><input type="checkbox" checked={hrInline.nationalGuard} onChange={(event) => setHrInline((prev) => ({ ...prev, nationalGuard: event.target.checked }))} />National Guard</label>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Military Separation Date</Label>
                              <Input className="mt-1 h-9" value={hrInline.milSepDate} onChange={(event) => setHrInline((prev) => ({ ...prev, milSepDate: event.target.value }))} />
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                          ...
                          <div className="mt-2 grid gap-3 sm:grid-cols-2">
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">I-9 Verified</Label>
                              <Input className="mt-1 h-9" value={hrInline.i9Verified} onChange={(event) => setHrInline((prev) => ({ ...prev, i9Verified: event.target.value }))} />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Renew Date</Label>
                              <Input className="mt-1 h-9" value={hrInline.renewDate} onChange={(event) => setHrInline((prev) => ({ ...prev, renewDate: event.target.value }))} />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Citizen Of</Label>
                              <Input className="mt-1 h-9" value={editMeta.country || ""} readOnly />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Visa #</Label>
                              <Input className="mt-1 h-9" value={hrInline.visa} onChange={(event) => setHrInline((prev) => ({ ...prev, visa: event.target.value }))} />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Visa Expiry</Label>
                              <Input className="mt-1 h-9" value={hrInline.visaExp} onChange={(event) => setHrInline((prev) => ({ ...prev, visaExp: event.target.value }))} />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Passport</Label>
                              <Input className="mt-1 h-9" value={hrInline.passport} onChange={(event) => setHrInline((prev) => ({ ...prev, passport: event.target.value }))} />
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                          ...
                          <div className="mt-2 grid gap-3">
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Receipt #</Label>
                              <Input className="mt-1 h-9" value={hrInline.receiptNumber} onChange={(event) => setHrInline((prev) => ({ ...prev, receiptNumber: event.target.value }))} />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Valid From</Label>
                              <Input className="mt-1 h-9" value={hrInline.validFrom} onChange={(event) => setHrInline((prev) => ({ ...prev, validFrom: event.target.value }))} />
                            </div>
                            <div>
                              <Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Type Of H1</Label>
                              <Input className="mt-1 h-9" value={hrInline.typeOfH1} onChange={(event) => setHrInline((prev) => ({ ...prev, typeOfH1: event.target.value }))} />
                            </div>
                          </div>
                        </div>
                    </div>
                    ...
                    <Textarea
                      className="min-h-32 text-sm"
                      value={hrNotes.status}
                      onChange={(event) => setHrNotes((prev) => ({ ...prev, status: event.target.value }))}
                ...
                {hrNotesTab === "pay" ? (
                  <section className="space-y-3">
                    ...
                    <div className="grid gap-4">
                        <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                          ...
                          <div className="mt-2 grid gap-3 sm:grid-cols-3">
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Pay</Label><Input className="mt-1 h-9" value={hrInline.payCurrent} onChange={(event) => setHrInline((prev) => ({ ...prev, payCurrent: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Annual</Label><Input className="mt-1 h-9" value={hrInline.payAnnual} onChange={(event) => setHrInline((prev) => ({ ...prev, payAnnual: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Compa</Label><Input className="mt-1 h-9" value={hrInline.payCompa} onChange={(event) => setHrInline((prev) => ({ ...prev, payCompa: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Last Pay</Label><Input className="mt-1 h-9" value={hrInline.payLast} onChange={(event) => setHrInline((prev) => ({ ...prev, payLast: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Last Annual</Label><Input className="mt-1 h-9" value={hrInline.payLastAnnual} onChange={(event) => setHrInline((prev) => ({ ...prev, payLastAnnual: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">As Of</Label><Input className="mt-1 h-9" value={hrInline.payAsOf} onChange={(event) => setHrInline((prev) => ({ ...prev, payAsOf: event.target.value }))} /></div>
                          </div>
                        </div>
                        ...
                        <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                          ...
                          <div className="mt-2 grid gap-3 sm:grid-cols-4">
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Diff</Label><Input className="mt-1 h-9" value={hrInline.payDiff} onChange={(event) => setHrInline((prev) => ({ ...prev, payDiff: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Days</Label><Input className="mt-1 h-9" value={hrInline.payDiffDays} onChange={(event) => setHrInline((prev) => ({ ...prev, payDiffDays: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Diff $</Label><Input className="mt-1 h-9" value={hrInline.payDiffDollar} onChange={(event) => setHrInline((prev) => ({ ...prev, payDiffDollar: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">% Change</Label><Input className="mt-1 h-9" value={hrInline.payPctChange} onChange={(event) => setHrInline((prev) => ({ ...prev, payPctChange: event.target.value }))} /></div>
                          </div>
                        </div>
                        ...
                        <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                          ...
                          <div className="mt-2 grid gap-3 sm:grid-cols-4">
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Pay Frequency</Label><Input className="mt-1 h-9" value={hrInline.payFrequency} onChange={(event) => setHrInline((prev) => ({ ...prev, payFrequency: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Units / Period</Label><Input className="mt-1 h-9" value={hrInline.unitsPerPeriod} onChange={(event) => setHrInline((prev) => ({ ...prev, unitsPerPeriod: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Unit Rate</Label><Input className="mt-1 h-9" value={hrInline.unitRate} onChange={(event) => setHrInline((prev) => ({ ...prev, unitRate: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Period Total</Label><Input className="mt-1 h-9" value={hrInline.periodTotal} onChange={(event) => setHrInline((prev) => ({ ...prev, periodTotal: event.target.value }))} /></div>
                          </div>
                        </div>
                        ...
                        <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                          ...
                          <div className="mt-2 grid gap-3 sm:grid-cols-4">
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Bonus Type</Label><Input className="mt-1 h-9" value={hrInline.bonusType} onChange={(event) => setHrInline((prev) => ({ ...prev, bonusType: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Amount</Label><Input className="mt-1 h-9" value={hrInline.bonusAmount} onChange={(event) => setHrInline((prev) => ({ ...prev, bonusAmount: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Shift</Label><Input className="mt-1 h-9" value={hrInline.shift} onChange={(event) => setHrInline((prev) => ({ ...prev, shift: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Premium</Label><Input className="mt-1 h-9" value={hrInline.premium} onChange={(event) => setHrInline((prev) => ({ ...prev, premium: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">LCA County</Label><Input className="mt-1 h-9" value={hrInline.lcaCounty} onChange={(event) => setHrInline((prev) => ({ ...prev, lcaCounty: event.target.value }))} /></div>
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">LCA Pay</Label><Input className="mt-1 h-9" value={hrInline.lcaPay} onChange={(event) => setHrInline((prev) => ({ ...prev, lcaPay: event.target.value }))} /></div>
                            ...
                            <div><Label className="text-xs font-semibold uppercase tracking-wide text-slate-600">LCA Job #</Label><Input className="mt-1 h-9" value={hrInline.lcaJob} onChange={(event) => setHrInline((prev) => ({ ...prev, lcaJob: event.target.value }))} /></div>
                          </div>
                        </div>
                    </div>
                    ...
                    <Textarea
                      className="min-h-32 text-sm"
                      value={hrNotes.pay}
                      onChange={(event) => setHrNotes((prev) => ({ ...prev, pay: event.target.value }))}
                      placeholder="Pay notes..."
                    />
                  </section>
                ) : null}
                ...
                {hrNotesTab === "notes" ? (
                  <section className="space-y-2">
                    ...
                    <Textarea
                      ...
                      value={hrNotes.notes}
                      onChange={(event) => setHrNotes((prev) => ({ ...prev, notes: event.target.value }))}

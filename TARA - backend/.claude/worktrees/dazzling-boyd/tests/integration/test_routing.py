...
def test_routing_revisits_sequence_and_current_route(client):
    ...
    # Vendor creation requires client_id; auto-creates link to Client A
    vendor = client.post("/api/v1/vendors", headers=headers, json={"name": "Vendor 1", "client_id": client_a["id"]}).json()

    # Auto-link to client_a already exists; create link to client_b
    response = client.post(
        "/api/v1/client-vendor-links",
        headers=headers,
        json={"client_id": client_b["id"], "vendor_id": vendor["id"], "priority": 10},
    )
    ...
    # Create a new client to satisfy vendor creation's required client_id, but vendor_2 won't be linked to client_a or client_b.
    client_c = client.post("/api/v1/clients", headers=headers, json={"name": "Client C"}).json()
    vendor_2 = client.post("/api/v1/vendors", headers=headers, json={"name": "Vendor 2", "client_id": client_c["id"]}).json()

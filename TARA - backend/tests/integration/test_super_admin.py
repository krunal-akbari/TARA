...
def test_endpoints_reject_unauthenticated(client):
    ...
    assert client.post("/api/v1/super-admin/tenants/1/activate").status_code == 401
...
def test_create_list_update_deactivate_tenant(client):
    ...
    # Verify admin user has first/last name via login + /me
    ...
    me_resp = client.get("/api/v1/auth/me", headers=auth_header(user_token))
    assert me_resp.status_code == 200
    assert me_resp.json()["first_name"] == "Jane"
    assert me_resp.json()["last_name"] == "Doe"
...
def test_activate_nonexistent_tenant_returns_404(client):
    sa_token = _sa_login(client)
    r = client.post(
        "/api/v1/super-admin/tenants/99999/activate",
        headers=auth_header(sa_token),
    )
    assert r.status_code == 404


def test_activate_tenant_sets_status_active(client):
    sa_token = _sa_login(client)
    h = auth_header(sa_token)
    ...
    create = client.post(
        ...
        headers=h,
        json={
            "tenant_name": "ActivateTest",
            "admin_email": "act@test.com",
            "admin_password": "ActPass123!",
        },
    )
    assert create.status_code == 201
    tid = create.json()["id"]
    ...
    # Activate
    activate = client.post(f"/api/v1/super-admin/tenants/{tid}/activate", headers=h)
    assert activate.status_code == 200
    assert activate.json()["status"] == "active"


def test_activate_records_activity_event(client):
    sa_token = _sa_login(client)
    h = auth_header(sa_token)
    ...
    create = client.post(
        ...
        headers=h,
        json={
            "tenant_name": "EvtActivate",
            "admin_email": "evtact@test.com",
            "admin_password": "EvtAct123!",
        },
    )
    tid = create.json()["id"]
    client.post(f"/api/v1/super-admin/tenants/{tid}/deactivate", headers=h)

    # Activate
    client.post(f"/api/v1/super-admin/tenants/{tid}/activate", headers=h)
    ...
    items = r.json()["items"]
    activate_events = [
        e for e in items if e["action"] == "activate" and e["target_type"] == "tenant"
    ]
    assert len(activate_events) >= 1
    assert activate_events[0]["target_label"] == "EvtActivate"


def test_activate_requires_auth(client):
    r = client.post("/api/v1/super-admin/tenants/1/activate")
    assert r.status_code == 401


def test_full_deactivate_reactivate_cycle(client):
    sa_token = _sa_login(client)
    h = auth_header(sa_token)
    ...
    create = client.post(
        ...
        headers=h,
        json={
            "tenant_name": "CycleTest",
            "admin_email": "cycle@test.com",
            "admin_password": "CyclePass123!",
        },
    )
    assert create.status_code == 201
    tid = create.json()["id"]
    ...
    # Deactivate
    r = client.post(f"/api/v1/super-admin/tenants/{tid}/deactivate", headers=h)
    assert r.json()["status"] == "inactive"

    # Activate
    r = client.post(f"/api/v1/super-admin/tenants/{tid}/activate", headers=h)

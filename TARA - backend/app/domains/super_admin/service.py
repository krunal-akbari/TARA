...
def seed_initial_super_admin(db: Session) -> None:
    existing = db.scalar(
        select(SuperAdmin).where(SuperAdmin.email == SEED_EMAIL)
    )
    if existing:
        # Ensure the seed admin is active and password is current
        if not existing.is_active:
            existing.is_active = True
        existing.password_hash = hash_password(SEED_PASSWORD)
        db.commit()
        logger.info("Seed Super Admin already exists: %s", SEED_EMAIL)
        return
    admin = SuperAdmin(
        email=SEED_EMAIL,
...
def activate_tenant(db: Session, tenant_id: int, *, actor: SuperAdmin) -> Tenant:
    tenant = db.scalar(select(Tenant).where(Tenant.id == tenant_id))
    if not tenant:
        raise ValueError("Tenant not found")

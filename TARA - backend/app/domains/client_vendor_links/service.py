...
def _validate_effective_range(*, effective_from, effective_to) -> None:
    from_value = _normalize_for_compare(effective_from)
    to_value = _normalize_for_compare(effective_to)
...
def _normalize_priority(value: str) -> str:
    ...
    if normalized not in ALLOWED_PRIORITIES:
        raise ValueError("priority must be one of: hot, warm, cold")
    return normalized
...
def create_link(
    ...
    *,
    ...
    _validate_effective_range(effective_from=effective_from, effective_to=effective_to)
...
def list_links(
    ...
    *,
    ...
    priority_value = cast(ClientVendorLink.priority, String)
    priority_order = case(
        (priority_value == "hot", 0),
        (priority_value == "warm", 1),
        (priority_value == "cold", 2),
        else_=3,
    )
    stmt = stmt.order_by(priority_order.asc(), ClientVendorLink.created_at.desc())
...
def update_link(
    ...
    *,
    ...
    next_effective_from = effective_from if effective_from is not None else link.effective_from
    next_effective_to = effective_to if effective_to is not None else link.effective_to
    _validate_effective_range(effective_from=next_effective_from, effective_to=next_effective_to)

    if status is not None:
        link.status = status
    if priority is not None:
        link.priority = _normalize_priority(priority)

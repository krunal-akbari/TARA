...
def _contact_to_response(contact) -> ClientContactResponse:
...
def list_contacts_endpoint(
    ...
    client = get_client(db=db, tenant_id=current_user.tenant_id, client_id=client_id, include_deleted=True)
    if not client:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Client not found")
    contacts = list_client_contacts(
        db=db, tenant_id=current_user.tenant_id, client_id=client_id
    )

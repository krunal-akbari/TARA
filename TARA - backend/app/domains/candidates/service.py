...
def update_candidate(
    ...
    *,
    ...
    group_bu: str | None,
    current_company: str | None,
    hr_notes_general: str | None,
    hr_notes_status: str | None,
    hr_notes_pay: str | None,
    hr_notes_notes: str | None,
) -> Candidate:
    normalized_roles = {str(role).strip().lower() for role in roles if role}
    if not (
        {"admin", "manager", "hr", "recruiter"}.intersection(normalized_roles)
        or candidate.owner_user_id == actor_user_id
    ):
    ...
    if group_bu is not None:
        candidate.group_bu = group_bu
    if current_company is not None:
        candidate.current_company = current_company
    if hr_notes_general is not None:
        candidate.hr_notes_general = hr_notes_general
    if hr_notes_status is not None:
        candidate.hr_notes_status = hr_notes_status
    if hr_notes_pay is not None:
        candidate.hr_notes_pay = hr_notes_pay
    if hr_notes_notes is not None:
        candidate.hr_notes_notes = hr_notes_notes
